...
- Added WP13-D–H presentation foundations: authoritative upgrade/Rebirth presenters, bounded prioritized notifications, semantic no-op audio cues pending reviewed assets, reduced-motion preferences, and deterministic major-event FOV pulses without gameplay or server changes.
...
- Completed WP-12's DataSync-only client factory presentation: cumulative `Workspace.FactoryEvolution` layer reconciliation, authored-property restoration, defensive render retry behavior, Rebirth-plus-stage notification gating, and final factory/Rebirth UI readability fixes without new remotes or gameplay changes.
